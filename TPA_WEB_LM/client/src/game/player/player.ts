...
import { Backward } from "./states/backward";
import { BackwardInverse } from "./states/backward-inverse";
...
export class Player extends Component{
    ...
    constructor() { 
        ...
        if(this.server.playerID === 1 || this.server.playerID === 100) {
    ...
    lowKick(){
        this.mediator.firstPlayerLowKick()
        this.kickLow = true
    }

    frontKick(){
        this.mediator.firstPlayerFrontKick(Action.RIGHT_FRONT_KICK)
        this.isFrontKick = true
    }

    frontKickInverse(){
        this.mediator.firstPlayerFrontKick(Action.LEFT_FRONT_KICK)
    ...
    configureState(){
        if(this.movingRight) {
            const orientation = this.mediator.firstPlayerOrientation()
            if(orientation == Action.INVERSE) {
                if(!(this.state instanceof BackwardInverse)) {
                    this.state = new BackwardInverse(this)
                }
            }
            else if(!(this.state instanceof WalkingState)) this.state = new WalkingState(this)
        }
        else if(this.movingLeft) {
            const orientation = this.mediator.firstPlayerOrientation()
            if(orientation == Action.NORMAL) {
                if(!(this.state instanceof Backward)) {
                    this.state = new Backward(this)
                }
            }
            else if(!(this.state instanceof WalkingStateInverse)) this.state = new WalkingStateInverse(this)
        }
        else if(this.isJumping) {
            const orientation = this.mediator.firstPlayerOrientation()
            if(orientation == Action.INVERSE) {
                if(!(this.state instanceof JumpingStateInverse)) this.state = new JumpingStateInverse(this)
                return
            }
